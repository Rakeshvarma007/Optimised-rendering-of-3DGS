...
def rasterization_lod(
    ...
    **kwargs,
    ...
    # Traverse Octree and collect selected Gaussians
    selected_means = []
    selected_scales = []
    selected_quats = []
    selected_opacities = []
    ...
    _lod_dbg = {
        "n_merged": 0,
        "n_leaf": 0,
        "merged_scale_raw_mean": None,
        "merged_scale_out_mean": None,
        "leaf_scale_mean": None,
        "merged_opacity_raw_mean": None,
        "merged_opacity_out_mean": None,
        "leaf_opacity_mean": None,
        "raw_inputs": bool(getattr(lod_octree, "raw_inputs", False)),
    }
    # #endregion
    ...
    orig_scales = scales
    orig_quats = quats
    orig_opacities = opacities
    orig_colors = colors

    # Check if colors contains Spherical Harmonics [N, K, 3] or direct color/features [N, D]
    has_sh = False
    K = 0
    if orig_colors is not None:
        if orig_colors.ndim >= 3:
            has_sh = True
    ...
    try:
        with open("debug-c361f6.log", "a", encoding="utf-8") as _agent_f:
            _agent_f.write(
                json.dumps(
                    {
                        "sessionId": "c361f6",
                        "runId": "post-fix",
                        "hypothesisId": "A",
                        "location": "rendering.py:rasterization_lod",
                        "message": "LOD selection scale/opacity stats",
                        "data": _lod_dbg,
    ...
    if len(selected_means) == 0:
        # Fallback: if the tree traversal yielded nothing, use original inputs
    ...
    # ===== DEFENSIVE DIMENSIONAL ALIGNMENT (Critical Safety Constraint) =====

    # Concat selected parameters
    means_lod = torch.cat(selected_means, dim=0)
    scales_lod = torch.cat(selected_scales, dim=0)
    quats_lod = torch.cat(selected_quats, dim=0)
    opacities_lod = torch.cat(selected_opacities, dim=0)

    # 1. Opacities: Force flat [N] vector — squeeze any trailing [N, 1] layout
    if opacities_lod.ndim == 2 and opacities_lod.shape[-1] == 1:
        opacities_lod = opacities_lod.squeeze(-1)
    assert opacities_lod.ndim == 1, (
        f"LOD opacities must be 1D [N], got shape {opacities_lod.shape}"
    )

    # 2. Scales: Ensure [N, 3] layout
    assert scales_lod.ndim == 2 and scales_lod.shape[-1] == 3, (
        f"LOD scales must be [N, 3], got shape {scales_lod.shape}"
    )

    # 3. Quats: Ensure [N, 4]
    assert quats_lod.ndim == 2 and quats_lod.shape[-1] == 4, (
        f"LOD quats must be [N, 4], got shape {quats_lod.shape}"
    )

    # 4. Colors/SH: Reconcile dimensionality between merged and original Gaussians
    colors_lod = None
    if orig_colors is not None and len(selected_sh) > 0:
        # Detect target shape from the first non-empty entry
        ref = selected_sh[0]
        ref_ndim = ref.ndim
        ref_trailing = ref.shape[1:]  # everything after the batch dim

        aligned_sh = []
        for i, sh_chunk in enumerate(selected_sh):
            chunk = sh_chunk
            # Case: SH [M, K, 3] vs direct features [M, D]
            if chunk.ndim != ref_ndim:
                if ref_ndim == 3 and chunk.ndim == 2:
                    # Expand direct [M, D] to [M, 1, D] then pad to [M, K, 3]
                    K_target = ref_trailing[0]
                    D_target = ref_trailing[1]
                    if chunk.shape[-1] == D_target:
                        chunk = chunk.unsqueeze(1)  # [M, 1, D]
                        if K_target > 1:
                            pad = torch.zeros(
                                (chunk.shape[0], K_target - 1, D_target),
                                dtype=chunk.dtype, device=chunk.device
                            )
                            chunk = torch.cat([chunk, pad], dim=1)
                    else:
                        # Trim or pad last dim to match
                        chunk = chunk[..., :D_target] if chunk.shape[-1] > D_target else F.pad(chunk, (0, D_target - chunk.shape[-1]))
                        ...
                        if K_target > 1:
                            ...
                            chunk = torch.cat([chunk, pad], dim=1)
                elif ref_ndim == 2 and chunk.ndim == 3:
                    # Collapse [M, K, 3] to [M, D] by taking DC component
                    chunk = chunk[:, 0, :]  # [M, 3]
                    D_target = ref_trailing[0]
                    if chunk.shape[-1] != D_target:
                        chunk = chunk[..., :D_target] if chunk.shape[-1] > D_target else F.pad(chunk, (0, D_target - chunk.shape[-1]))

            # Ensure trailing dims match reference (handle K or D mismatches within same ndim)
            if chunk.ndim == ref_ndim and chunk.shape[1:] != ref_trailing:
                if ref_ndim == 3:
                    K_target, D_target = ref_trailing
                    K_cur = chunk.shape[1]
                    D_cur = chunk.shape[2]
                    # Pad/trim K dimension
                    if K_cur < K_target:
                        pad = torch.zeros(
                            (chunk.shape[0], K_target - K_cur, D_cur),
                            dtype=chunk.dtype, device=chunk.device
                        )
                        chunk = torch.cat([chunk, pad], dim=1)
                    elif K_cur > K_target:
                        chunk = chunk[:, :K_target, :]
                    # Pad/trim D dimension
                    if D_cur != D_target:
                        chunk = chunk[..., :D_target] if D_cur > D_target else F.pad(chunk, (0, D_target - D_cur))
                elif ref_ndim == 2:
                    D_target = ref_trailing[0]
                    D_cur = chunk.shape[-1]
                    chunk = chunk[..., :D_target] if D_cur > D_target else F.pad(chunk, (0, D_target - D_cur))

            aligned_sh.append(chunk)

        colors_lod = torch.cat(aligned_sh, dim=0)

    # ===== END DEFENSIVE DIMENSIONAL ALIGNMENT =====

    # Call original rasterization with aligned tensors
    return rasterization(
        means=means_lod,
        quats=quats_lod,
        scales=scales_lod,
