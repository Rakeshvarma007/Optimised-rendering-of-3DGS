...
__device__ inline float round_floor(float x) { return floorf(x); }
__device__ inline float round_ceil(float x) { return ceilf(x); }

namespace {
    template <float (*RoundFn)(float)>
...
__global__ void intersect_tile_lidar_kernel(
    ...
    const int min_dense_el = sample_dense_el<round_floor>(beg_el, fov_span_pix_el, raycdf_size_el);
    const int max_dense_el = sample_dense_el<round_ceil>(end_el, fov_span_pix_el, raycdf_size_el);
    ...
    // Sample A azimuth.
    const int begA_dense = sample_dense_az<round_floor>(begA_pix_az, fov_span_pix_az, raycdf_size_az);
    const int endA_dense = sample_dense_az<round_ceil>(endA_pix_az, fov_span_pix_az, raycdf_size_az);
    ...
    if(has_raysA)
    {
        begA_tile = sample_tile_az<round_floor>(begA_pix_az, fov_span_pix_az, n_bins_az);
        endA_tile = sample_tile_az<round_ceil>(endA_pix_az, fov_span_pix_az, n_bins_az);
    }
    ...
    if (underflows || overflows)
    {
        ...
        const int begB_dense = sample_dense_az<round_floor>(begB_pix_az, fov_span_pix_az, raycdf_size_az);
        const int endB_dense = sample_dense_az<round_ceil>(endB_pix_az, fov_span_pix_az, raycdf_size_az);
        has_raysB = has_any_rays_in_tile(lidar, raycdf_size_el, raycdf_size_az,
                                         min_dense_el, max_dense_el, begB_dense, endB_dense);
        if(has_raysB)
        {
            begB_tile = sample_tile_az<round_floor>(begB_pix_az, fov_span_pix_az, n_bins_az);
            endB_tile = sample_tile_az<round_ceil>(endB_pix_az, fov_span_pix_az, n_bins_az);
        }
    }
