...
# Removes PyTorch's injected NO_HALF flags to allow compiling CUB code on Windows.
for flag in [
    ...
    if flag in jit.COMMON_NVCC_FLAGS:
        jit.COMMON_NVCC_FLAGS.remove(flag)

if hasattr(jit, "_add_gnu_cpp_abi_flag"):
    original_add_gnu_cpp_abi_flag = jit._add_gnu_cpp_abi_flag
    def patched_add_gnu_cpp_abi_flag(cflags):
        ...
        for flag in [
            ...
            while flag in cflags:
                cflags.remove(flag)
    jit._add_gnu_cpp_abi_flag = patched_add_gnu_cpp_abi_flag
# ---------------------------------------------
try:
