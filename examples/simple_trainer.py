...
from gsplat.rendering import rasterization_lod
...
import experimental
from lod_octree import GaussianLODBuilder  # <--- ADD THIS LINE
...
class Runner:
    ...
    def __init__(
        ...
        self.lod_octree = None
        self._last_lod_num_gaussians = 0
        if cfg.enable_lod:
            self.lod_octree = GaussianLODBuilder(
                points=self.splats["means"],
                scales=self.splats["scales"],
                quats=self.splats["quats"],
                opacities=self.splats["opacities"],
                sh0=self.splats["sh0"],
                shN=self.splats["shN"] if "shN" in self.splats else None,
                max_depth=cfg.lod_max_depth,
                min_gaussians=cfg.lod_min_gaussians,
                raw_inputs=True,
            )
            self.lod_octree.build()
            self._last_lod_num_gaussians = len(self.splats["means"])
    ...
    def rasterize_splats(
        ...
        **kwargs,
        ...
        if self.cfg.enable_lod and self.lod_octree is not None:
            render_colors, render_alphas, info = rasterization_lod(
                lod_octree=self.lod_octree,
                means=means,
                quats=quats,
                scales=scales,
                opacities=opacities,
                ...
                width=width,
                height=height,
                lod_threshold=self.cfg.lod_threshold,
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            if cfg.enable_lod:
                if step % 100 == 0 or len(self.splats["means"]) != self._last_lod_num_gaussians:
                    self.lod_octree = GaussianLODBuilder(
                        points=self.splats["means"],
                        scales=self.splats["scales"],
                        quats=self.splats["quats"],
