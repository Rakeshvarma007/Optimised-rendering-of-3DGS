...
from gsplat.rendering import rasterization, rasterization_lod
...
def main(local_rank: int, world_rank, world_size: int, args):
    ...
    # Build LOD Octree if enable_lod is True
    lod_octree = None
    if args.enable_lod:
        from lod_octree import GaussianLODBuilder
        # Split colors/sh into sh0 and shN if needed
        splats = gaussian_scene.splats
        colors_val = splats.get("colors", splats.get("sh0"))
        if colors_val.ndim == 3:
            sh0 = colors_val[:, :1]
            shN = colors_val[:, 1:] if colors_val.shape[1] > 1 else None
        else:
            sh0 = colors_val
            shN = None

        lod_octree = GaussianLODBuilder(
            points=splats["means"],
            ...
            sh0=sh0,
            shN=shN,
            max_depth=args.lod_max_depth,
            min_gaussians=args.lod_min_gaussians,
            raw_inputs=True,
        )
        lod_octree.build()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--enable_lod", action="store_true", help="enable LOD spatial hierarchy rendering"
    )
    parser.add_argument(
        "--lod_max_depth", type=int, default=4, help="maximum depth of the LOD Octree"
    )
    parser.add_argument(
        "--lod_min_gaussians", type=int, default=8, help="minimum number of Gaussians in an LOD node before splitting"
    )
    parser.add_argument(
